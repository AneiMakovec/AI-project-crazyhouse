...
#     0xF8D626AAAF278509, 0xb34799dddb8c5696, 0x8e507d5c95cbbb9d, 0x89f1e0319d760395,
#     0x9fdf6649530532b8, 0x0931934a72d77fb8, 0x73846f37ef70e7a9, 0x2dfe31c38d5a4399,
#     0xc28b04f57eb80771, 0x80abd90062a48907, 0xf92391c1f8334597, 0x21445b936f346d50,
#     0xf6c31e947783702b, 0x3073aa3f2b04d61a, 0xf4433d0ff159a316, 0x9d4014e0e2220079,
#     0x041904f4dc9918c9, 0xdfdd11052dc1437b, 0x310b4cbad0db2554, 0x398934de72644c3e,
#     0xce3c31773635fb34, 0xd3a99c6dde2e6e9a, 0x6afeb706bd942c7e, 0x3c1e21f7d97506ea,
#     0x51fe5482a22be1a4, 0x4e7724ad27bd5393, 0xfe0011575e02e975, 0x2debbc44de24ebe2,
#     0xb10f600a2682f526, 0x27ed3a4468f167a7, 0x6a7997c1f59114fc
# ]
...
class GameBoard(chess.variant.CrazyhouseBoard):
    ...
    def __init__(self, fen: Optional[str] = starting_fen, chess960: bool = False) -> None:
        ...
        # self.hash = self._init_hash()
        # self.mirrored = False

    def push(self, move: chess.Move) -> None:
        # recalculate hash
        # if move.drop:
        #     dropped_piece = move.drop - 1
        #     pocket_count = self.pockets[self.turn].count(move.drop)
        #     new_pocket_count = pocket_count - 1
        #     piece_offset = 781
        #
        #     if move.drop == chess.KNIGHT:
        #         piece_offset += 16
        #     elif move.drop == chess.ROOK:
        #         piece_offset += 20
        #     elif move.drop == chess.BISHOP:
        #         piece_offset += 24
        #     elif move.drop == chess.QUEEN:
        #         piece_offset += 28
        #
    ...
    #     state, _ = self.fen().split(None, 1)
    #     if state in self.state_repetitions and self.state_repetitions[state] >= 5:
    #         return chess.Outcome(chess.Termination.FIVEFOLD_REPETITION)
    #     else:
    ...
    #             ep_mask = chess.shift_down(chess.BB_SQUARES[self.ep_square])
    #         else:
    #             ep_mask = chess.shift_up(chess.BB_SQUARES[self.ep_square])
    #         ep_mask = chess.shift_left(ep_mask) | chess.shift_right(ep_mask)
    #
    #         if ep_mask & self.pawns & self.occupied_co[self.turn]:
    #             zobrist_hash ^= RANDOM_ARRAY[772 + chess.square_file(self.ep_square)]
    #
    #     # turn
    #     zobrist_hash ^= RANDOM_ARRAY[780] if self.turn else 0
    #
    #     # pocket count
    #     for player in [chess.WHITE, chess.BLACK]:
    #         for piece in range(chess.PAWN, chess.KING):
    #             pocket_count = self.pockets[player].count(piece)
    #
    #             if pocket_count > 0:
    #                 pocket_count -= 1
    #
    #                 if piece == chess.PAWN:
    #                     zobrist_hash ^= RANDOM_ARRAY[781 + pocket_count]
    #                 elif piece == chess.KNIGHT:
    #                     zobrist_hash ^= RANDOM_ARRAY[781 + 16 + pocket_count]
    #                 elif piece == chess.ROOK:
    #                     zobrist_hash ^= RANDOM_ARRAY[781 + 20 + pocket_count]
    #                 elif piece == chess.BISHOP:
    #                     zobrist_hash ^= RANDOM_ARRAY[781 + 24 + pocket_count]
    #                 else:
    #                     zobrist_hash ^= RANDOM_ARRAY[781 + 28 + pocket_count]
    #
    #     return zobrist_hash
